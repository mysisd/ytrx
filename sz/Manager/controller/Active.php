...
use app\manager\model\Active as  RegardModel;

class Active extends Base{

//    内容管理的关于我们的数据遍历和删除
    public function yt_active(){
        if(input('delete')!=''){
            $row = RegardModel::row();
            if($row){
                $this->success('删除成功！','yt_active');
            }else{
                $this->error('删除失败！请重新删除','yt_active');
            }
        }else{
...
//    修改信息
    public function update_active(){
        if(input('id')!=''){
            ...
            if($row !== false){
                $this->success('修改成功！','yt_active');
            }else{
                $this->error('修改失败！请重新修改...',"/yt_active/id/".$id);
    ...
    public function add_active(){
        ...
        if(empty($arr)){
           ...
           if(input('serial')!=''){
            ...
            if($row){
                $this->success('添加成功！','yt_active');
            }else{
                $this->error('添加失败！请重新添加...','add_active');
